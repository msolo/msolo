...
int FCGX_GetChar(FCGX_Stream *stream)
{
    ...
    stream->fillBuffProc(stream, NULL);
    if (stream->isClosed)
        return EOF;
 ...
 *----------------------------------------------------------------------
 */
static void FillBuffProc(FCGX_Stream *stream, struct timeval* timeout)
{
    FCGX_Stream_Data *data = (FCGX_Stream_Data *)stream->data;
    FCGI_Header header;
    int headerLen = 0;
    ...
    for (;;) {
        ...
        if(stream->rdNext == data->buffStop) {
            if (timeout) {
              count = OS_ReadWithTimeout(data->reqDataPtr->ipcFd, (char *)data->buff,
                                         data->bufflen, timeout);
            }
            else {
                     ...
                     int acceptInputTimeout)
{
    ...
    request->ipcFd = -1;
    request->acceptInputTimeout = acceptInputTimeout;
...
int FCGX_Init(void)
{
    ...
    FCGX_InitRequest(&the_request, FCGI_LISTENSOCK_FILENO, 0, 0);
...
int FCGX_Accept_r(FCGX_Request *reqDataPtr)
{
    ...
    for (;;) {
        ...
        if (reqDataPtr->acceptInputTimeout) {
          struct timeval temp_timeout;
          temp_timeout.tv_sec = reqDataPtr->acceptInputTimeout;
          temp_timeout.tv_usec = 0;
          timeout = &temp_timeout;
        }
        reqDataPtr->in = NewReader(reqDataPtr, 8192, 0);
        FillBuffProc(reqDataPtr->in, timeout);
