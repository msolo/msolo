...
class SparrowParser(Parser):
    ...
    def placeholder(self):
        START_PLACEHOLDER = self._scan('START_PLACEHOLDER')
        if self._peek('ID', 'SPACE', 'ASSIGN_OPERATOR', 'DOT', 'OPEN_PAREN', 'OPEN_BRACKET', '"[ \\t]*,[ \\t]*"', "'[ \\t]*in[ \\t]*'", 'CLOSE_PAREN', 'CLOSE_BRACKET', "'[ \\t]*\\*[ \\t]*'", "'[ \\t]*\\/[ \\t]*'", "'[ \\t]*\\%[ \\t]*'", "'[ \\t]*\\+[ \\t]*'", "'[ \\t]*\\-[ \\t]*'", 'COMP_OPERATOR', "'[ \\t]*and[ \\t]*'", "'[ \\t]*or[ \\t]*'", 'CLOSE_DIRECTIVE', "'[ \\t]*:[ \\t]*'", 'CLOSE_BRACE') == 'ID':
            ID = self._scan('ID')
    ...
    def literal(self):
        ...
        if _token_ != 'NUM':
            ...
            NUM = self._scan('NUM')
            int_part = NUM
            if self._peek('"\\."', 'CLOSE_DIRECTIVE', 'DOT', 'OPEN_PAREN', 'OPEN_BRACKET', '"[ \\t]*,[ \\t]*"', "'[ \\t]*\\*[ \\t]*'", 'CLOSE_PAREN', "'[ \\t]*\\/[ \\t]*'", "'[ \\t]*\\%[ \\t]*'", "'[ \\t]*\\+[ \\t]*'", "'[ \\t]*\\-[ \\t]*'", 'COMP_OPERATOR', "'[ \\t]*and[ \\t]*'", "'[ \\t]*or[ \\t]*'", 'CLOSE_BRACKET', "'[ \\t]*:[ \\t]*'", 'ASSIGN_OPERATOR', 'ID', 'CLOSE_BRACE') == '"\\."':
                self._scan('"\\."')
                NUM = self._scan('NUM')
    ...
    def primary(self, in_placeholder_context=False):
        ...
        while self._peek('DOT', 'OPEN_PAREN', 'OPEN_BRACKET', "'[ \\t]*\\*[ \\t]*'", "'[ \\t]*\\/[ \\t]*'", "'[ \\t]*\\%[ \\t]*'", "'[ \\t]*\\+[ \\t]*'", "'[ \\t]*\\-[ \\t]*'", 'COMP_OPERATOR', "'[ \\t]*and[ \\t]*'", "'[ \\t]*or[ \\t]*'", 'CLOSE_DIRECTIVE', 'CLOSE_BRACKET', '"[ \\t]*,[ \\t]*"', "'[ \\t]*:[ \\t]*'", 'ASSIGN_OPERATOR', 'ID', 'CLOSE_PAREN', 'CLOSE_BRACE') in ['DOT', 'OPEN_PAREN', 'OPEN_BRACKET']:
            _token_ = self._peek('DOT', 'OPEN_PAREN', 'OPEN_BRACKET')
            if _token_ == 'DOT':
                DOT = self._scan('DOT')
                ID = self._scan('ID')
    ...
    def argument_list(self):
        ...
        expression = self.expression()
        _arg = expression
        while self._peek('"[ \\t]*,[ \\t]*"', 'ASSIGN_OPERATOR', 'ID', 'CLOSE_PAREN') == '"[ \\t]*,[ \\t]*"':
            self._scan('"[ \\t]*,[ \\t]*"')
            _pargs.append(_arg)
            expression = self.expression()
            _arg = expression
        if self._peek('ASSIGN_OPERATOR', 'ID', 'CLOSE_PAREN') == 'ASSIGN_OPERATOR':
            ASSIGN_OPERATOR = self._scan('ASSIGN_OPERATOR')
            ...
            _kargs.append(_karg)
            while self._peek('ID', 'CLOSE_PAREN') == 'ID':
                identifier = self.identifier()
