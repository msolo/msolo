...
int OS_ReadWithTimeout(int fd, char * buf, size_t len, struct timeval* timeout)
{
  fd_set read_fds;
  ...
  FD_ZERO(&read_fds);
  FD_SET(fd, &read_fds);
  /* fixme: this code is not good */
  for (; retry; retry--)
  {
    ready_fd_count = select(fd + 1, &read_fds, NULL, NULL, timeout);
    if (ready_fd_count < 0) {
      if (errno != EINTR) {
      ...
      /* fixme: evil write to global data */
      errno = ETIMEDOUT;
      return -1;
    }
    rc = read(fd, buf, len);
    if (rc < 0 && OS_Errno == EAGAIN) {
      continue;
    }
    else {
      return rc;
